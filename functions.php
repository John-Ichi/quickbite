...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['store_registration'])) { // Store registration
    ...
    try {
        if ($register_store->execute()) {
            ...
            @file_put_contents(__DIR__ . '/auth_debug.log', date('c') . " - store_registration: success for {$email}\n", FILE_APPEND);
            header('Location: store_login.php');
            exit();
        }
    } catch (mysqli_sql_exception $e) {
        // Log exception for debugging
        @file_put_contents(__DIR__ . '/auth_debug.log', date('c') . " - store_registration: exception: " . $e->getMessage() . " -- POST:" . json_encode($_POST) . "\n", FILE_APPEND);
        if ($e->getCode() === 1062) {
            ...
            header('Location: store_register.php');
            // Log generic error
            @file_put_contents(__DIR__ . '/auth_debug.log', date('c') . " - store_registration: other error: " . $e->getMessage() . "\n", FILE_APPEND);
            exit();
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['store_login'])) { // Store login
    ...
    try {
        ...
        if (!password_verify($password,$info['password'])) {
            ...
            @file_put_contents(__DIR__ . '/auth_debug.log', date('c') . " - store_login: wrong password for {$email}\n", FILE_APPEND);
            storeLoginErr('Error: Wrong password.', $email);
        }

        // Successful login
        @file_put_contents(__DIR__ . '/auth_debug.log', date('c') . " - store_login: success for {$email}\n", FILE_APPEND);
...
function getStudentOrders($student_number = null) { // Helper function to fetch student orders
    $conn = connect();
    
    // If no student_number passed, try to get from session
    if ($student_number === null && isset($_SESSION['student_number'])) {
        $student_number = $_SESSION['student_number'];
    }
    
    if ($student_number === null) {
        return; // Exit if no student_number available
    }

    $sql = "SELECT o.id, o.checkout_id, o.total_sale, o.payment_method, o.status, o.created_at, o.status_updated_at
            FROM orders o
            WHERE o.customer_id = ?
            ORDER BY o.created_at DESC";
    
    $get_orders = $conn->prepare($sql);
    $get_orders->bind_param('s', $student_number);

    try {
        if ($get_orders->execute()) {
            $rs = $get_orders->get_result();

            if ($rs->num_rows === 0) {
                $output = json_encode(null);
            } else {
                $orders = [];
                while ($order = $rs->fetch_assoc()) {
                    $orders[] = $order;
                }
                $output = json_encode($orders, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
            }
            file_put_contents('student_orders.json', $output);
        }
    } catch (mysqli_sql_exception $e) {
        setcookie('get_orders_err', 'Database error: ' . $e->getMessage());
    }
}
?>